...
Lógica de negocio para quiz de ciberseguridad.
"""
...
class QuizService:
    """Servicio para gestionar quiz de ciberseguridad."""

    # Banco de preguntas fijas
    QUESTION_BANK = [
        {
            "id": "q001",
    ...
    FIXED_QUESTIONS_COUNT = 7
    AI_QUESTIONS_COUNT = 3

    @staticmethod
    def load_quiz() -> list:
        """
        Carga el quiz completo: preguntas fijas + generadas por IA.

        Returns:
            lista de diccionarios con todas las preguntas
        """
        # Seleccionar preguntas fijas aleatorias
        fixed = random.sample(QuizService.QUESTION_BANK, QuizService.FIXED_QUESTIONS_COUNT)
        ...
        if len(ai_questions) < QuizService.AI_QUESTIONS_COUNT:
            remaining = [q for q in QuizService.QUESTION_BANK if q not in fixed]
            ai_questions = random.sample(remaining, min(QuizService.AI_QUESTIONS_COUNT, len(remaining)))

        # Marcar preguntas de IA
    ...
    def submit_answer(question_id: str, answer: str) -> dict:
        ...
        Returns:
            dict con resultado de la respuesta
        """
        questions = session.get('quiz_questions', [])
        ...
        answers[question_id] = {'given': answer.upper(), 'correct': correct, 'is_correct': is_correct}
    ...
    def calculate_results() -> dict:
        """
        Calcula y guarda los resultados finales del quiz.

        Returns:
            dict con resultados completos para template
        """
        questions = session.get('quiz_questions', [])
        answers = session.get('quiz_answers', {})

        if not questions:
            return {"error": "No hay quiz en progreso"}
        ...
        try:
            ...
            logger.exception("Error guardando QuizResult: %s", e)
            db.session.rollback()
    ...
    def get_user_history(user_id: int = None) -> list:
        """Obtiene historial de quiz del usuario."""
        if user_id is None:
            user_id = current_user.id
    ...
    def get_user_stats(user_id: int = None) -> dict:
        """Obtiene estadísticas de quiz del usuario."""
        if user_id is None:
            user_id = current_user.id
        ...
        level_dist = {
            "experto": sum(1 for r in results if r.score >= 90),
            "avanzado": sum(1 for r in results if 70 <= r.score < 90),
            "intermedio": sum(1 for r in results if 50 <= r.score < 70),
            "principiante": sum(1 for r in results if r.score < 50),
        }
